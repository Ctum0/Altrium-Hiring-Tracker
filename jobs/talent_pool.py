...
when the new job has a structured domain, match that domain. Candidates who
already applied to the new job are never suggested.
"""
...
def find_suggestions(job, limit=SUGGESTION_LIMIT):
    ...
    status 'rejected' or 'on_hold' and shortlist_score >= 80; when the new
    job's domain is not 'other', the source job must share that domain;
    the candidate must not already have an application on the new job.
