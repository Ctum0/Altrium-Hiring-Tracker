...
def synthesize_panel_consensus(application):
    ...
    is_divergent = (hire_votes > 0 and reject_votes > 0) or (score_spread >= 3.5)
    ...
    if any(k in all_notes_text for k in ['python', 'backend', 'code', 'django', 'architecture', 'technical']):
    ...
    if any(k in all_notes_text for k in ['experience', 'senior', 'solid', 'problem']):
        agreed_strengths.append('Proven industry experience and problem-solving capability.')
        ...
        for ev in evaluators:
            if ev['vote'] == 'Hire':
                conflict_points.append(
                    f"{ev['name']} ({ev['round_name']}, Score: {ev['score_10']}/10) recommended HIRE based on strong role skills."
                )
            elif ev['vote'] == 'Reject':
                conflict_points.append(
    ...
    if is_divergent:
        recommendation = (
            f"Panel divergence detected ({hire_votes} Hire vs. {reject_votes} Reject). "
        ...
        recommendation = (
            f"Strong panel alignment across {len(feedbacks)} round(s). "
